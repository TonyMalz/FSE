...
public class PrettyTreeTansducer {
	...
	private String getOutputTreeRec(Node node, int level){
		...
		StringBuilder out = new StringBuilder(String.valueOf(node.id) + " : " + dataOrOp + " -> " + result + "\n");
		for(Node child : node.children) {
			out.append(linePrefix + "|\n");
			out.append(linePrefix + childPrefix + getOutputTreeRec(child,level+1));
		}
	...
	private Node parseInput(ArrayList<String> input,boolean printDebug) throws ParseException {
			...
			if (m.find()){
				...
				if (parsedNodes.containsKey(node.id)) {
					...
					if (n.isInitialized == false) {
						...
						throw new ParseException("INVALID TOKEN: node with same id was already set: " + line );
						//continue;
					}
				}
				...
				if (node.isRoot && foundRoot == true){
					...
					throw new ParseException("INVALID TOKEN: only one root node is allowed: " + line );
					//continue;
				} else if (node.isRoot) {
					foundRoot = true;
					root = node;
					...
					if (node.id == node.parentId){
						...
						throw new ParseException("INVALID TOKEN: cyclic reference id == parent id in line: " + line );
						//continue;
					}
				...
				throw new ParseException("INVALID TOKEN: invalid format in line: " + line );
				//continue;
			}
		}
		...
		if (foundRoot == false) {
			throw new ParseException("PARSE ERROR: no root element was specified!");
		}
		...
		for (Node parsedNode : parsedNodes.values()) {
			if (parsedNode.isInitialized == false){
				...
				throw new ParseException("PARSE ERROR: missing parent node with id: " + parsedNode.id );
			}
		}
